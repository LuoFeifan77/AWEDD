...
% alpha=4;%[2,4,8,10,20,30,50,100]; % [1,2,4,8,10,20,30,50,100];%default   alpha=4鏄渶浼樺弬鏁?
alpha=4;
...
DESC_DIR_path{2}           =fullfile('G:\Test_Data\test_remesh_SCAPE\AWEDS_non_norm_desc968');  %

%  娴嬭瘯remesh TOSCA鏁版嵁闆?
SHAPE_DIR_path{3}           =fullfile('I:\dataset\TOSCA_remesh\TOSCA_offset');  %
DESC_DIR_path{3}           =fullfile('G:\Test_Data\test_remesh_TOSCA\AWEDS_new_weight_desc');  %
%--------------------娴嬭瘯鍙傛暟alpha--------------------------
% SHAPE_DIR=fullfile('E:\MatlabWorkPlace\TwoType_WEDS-master\DATA_AND_TEST\Data-Faust\offset'); 
SHAPE_DIR_path{4}           =fullfile('H:\data_new\SHERC19\off_2');  %
DESC_DIR_path{4}           =fullfile('H:\Test_Data\test_SHREC\AWEDS_desc');  %
for t=3:3
    %--------------鏀硅繖涓尯鍩?---------------------%
...
%     DESC_DIR=fullfile('G:\test_param\alpha\non_norm_part',['alpha_',num2str(alpha(t))]);
    ...
    if sample <= Nscales   %16  闃叉璺戝嚭鏈?澶х淮搴?1024=32*32  Ndim=xdim*samples
        
        k_scale = floor(linspace((Nscales+1) ,1,sample));% 鍘熷閲囨牱鎯呭喌
    ...
    for s = 2:length(SHAPES) %杩愯鍚庨潰90-99涓ā鍨?
        ...
        time_start = tic;
        desc = compute_AWEDS_new_weight(shape,param);
